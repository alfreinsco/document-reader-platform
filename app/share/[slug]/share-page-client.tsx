...
export function SharePageClient({ slug }: { slug: string }) {
  const [shareUrl, setShareUrl] = useState(() => {
    if (typeof window === 'undefined') return '';
    return `${window.location.origin}${window.location.pathname.replace(/\/share\/[^/]+$/, '')}/read/${slug}`;
  });

  useEffect(() => {
    setShareUrl(
      `${window.location.origin}${window.location.pathname.replace(/\/share\/[^/]+$/, '')}/read/${slug}`
    );
  }, [slug]);
